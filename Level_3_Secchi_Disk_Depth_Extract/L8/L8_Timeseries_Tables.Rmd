...
extract_sites <- read.csv(paste0(L8_Tabs, 'site_sa.csv'))
extract_random <- read.csv(paste0(L8_Tabs, 'random_sa.csv'))
extract_stratified <- read.csv(paste0(L8_Tabs, 'strat_sa.csv'))
extract_regular <- read.csv(paste0(L8_Tabs, 'regular_sa.csv'))
...
names(meta_info) <- c("L8_filename")

#Get meta data from filename
meta_info <- add_column(meta_info, Year = round(as.numeric(substr(meta_info$L8_filename,18,21)), 0), .after = 'L8_filename')
meta_info <- add_column(meta_info, Month = sprintf("%02d", round(as.numeric(substr(meta_info$L8_filename, 22, 23)),0)), .after = 'Year')
meta_info <- add_column(meta_info, Day = sprintf("%02d", round(as.numeric(substr(meta_info$L8_filename, 24, 25)), 0)), .after = 'Month')
meta_info <- add_column(meta_info, Date = as.Date(paste0(meta_info$Day,"-", meta_info$Month, "-", meta_info$Year), "%d-%m-%Y"), .after = 'L8_filename')
...
write.csv(site_ts,paste0(L8_Tabs, 'site_ts.csv'), row.names = F)
write.csv(rand_ts,paste0(L8_Tabs, 'random_ts.csv'), row.names = F)
write.csv(strat_ts,paste0(L8_Tabs, 'stratified_ts.csv'), row.names = F)
write.csv(reg_ts,paste0(L8_Tabs, 'regular_ts.csv'), row.names = F)
...
#Write time series dataframe to output directory
write.csv(site_stats,paste0(L8_Tabs, 'site_stats.csv'), row.names = F)
write.csv(rand_stats,paste0(L8_Tabs, 'random_stats.csv'), row.names = F)
write.csv(strat_stats,paste0(L8_Tabs, 'stratified_stats.csv'), row.names = F)
write.csv(reg_stats,paste0(L8_Tabs, 'regular_stats.csv'), row.names = F)
